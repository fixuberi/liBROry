...
RSpec.describe "Books page:" do
  ...
  describe "With book_editor permission" do
    before do
  ...
  describe "WithOUT book_editor permission" do
    before { visit root_path }

    let!(:book) { FactoryGirl.create(:book, authors: [author], groups: [group]) }
    it { expect(page).to have_link "Books", href: books_path }

    describe "when GET to Books list" do
      before { visit books_path }
      it "Book page without New_book link" do
        expect(page).to have_current_path(books_path)
        expect(page).not_to have_link "New book", href: new_book_path
      end
    end

    describe "when GET to Book page" do
      before { visit book_path(book) }
      it "Book page without Edit and Delete links" do
    ...
    describe "when GET to edit Book" do
      before { visit edit_book_path(book) }
    ...
    describe "when GET to new Book" do
      before { visit new_book_path }
      it "should redirect with alert message" do
        expect(page).to have_current_path(root_path)
        expect(page).to have_error_message "not authorized"
