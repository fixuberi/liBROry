...
RSpec.describe "Author pages" do
  ...
  describe "As NON-ADMIN" do
    let!(:author) { FactoryGirl.create(:author) }
    it { expect(page).to have_link "Authors", href: authors_path }

    describe "when GET to Authors list" do
      before { visit authors_path }
      it { should have_current_path(authors_path) }
    end

    describe "when GET to author page" do
      before { visit author_path(author) }
      it { should have_current_path(author_path(author)) }
    end

    describe "when GET to edit author" do
      before { visit edit_author_path(author) }
    ...
    describe "when GET to new author" do
      before { visit new_author_path }
      it "should redirect with alert message" do
        expect(page).to have_current_path(root_path)
        expect(page).to have_error_message "not authorized"
